...
* クライアントからのリクエストとリアルサーバからのレスポンスはロードバランサを経由しなければならない  
* 各WebサーバのゲートウェイにはロードバランサのIPアドレスを設定しておく  
...
#####NAT構成  
L4スイッチはクライアントから届いたパケットの送信先アドレスを書き換えてリアルサーバに転送する。  
よって応答パケットを受け取ってIPアドレスを書き戻す必要がある。  

#####DSR構成  
L4スイッチはIPアドレスの書き換えは行わなわず、クライアントから受け取ったパケットをそのままリアルサーバへルーティングする。  
この場合、応答パケットに対してIPアドレスを書き戻す必要がないのでリアルサーバはL4スイッチを経由せずに応答を返すことができる。  
ロードバランサがボトルネックになったり、高トラフィックに耐える負荷分散環境が必要な場合はDSR構成が良い。  
ただしDSR構成では仮想サーバのパケットがリアルサーバにそのまま到達するので、__リアルサーバがグローバルアドレスを処理できなければならない。__  
...
VRRPパケットには  
   ...
   * IP Address（VIP）  
   * Virtual Rtr ID（仮想ルータID）  
   * Priority（プライオリティ）  

などのデータが格納されている。  
BackupノードはVRRPパケットを受信できている限り待機しているが、一定時間VRRPパケットを受信できなければマスタノードがダウンしたと判断してフェイルオーバを開始する。  

#####仮想ルータID  
VRRPでは仮想ルータIDというパラメータでインスタンスを分けることができるので、一つのネットワーク上にある複数の系列のロードバランサが同じアドレスにVRRPパケットを送出しても問題ない。  

#####プライオリティ
VRRPではプライオリティという値をノードごとに設定できる。  
これにより各ノードはVRRPパケットを受信できなくなると、自分からVRRPパケットを送出し始め、自分よりも高いプライオリティを持ったノードがいるかどうか確認できる。自分よりも高いプライオリティを持ったノードが見つかった時点でマスタノードになることを断念する。  

#####プリエンプティブモード  
VRRPのデフォルト設定では既存のマスタノードよりも高いプライオリティを持ったノードが起動するとフェイルオーバが発生する。  
この挙動はプリエンプティブモードによって変更できる。マスタノードの調子が悪く、頻繁に再起動を繰り返してしまうような恐れがある場合はプリエンプティブモードを無効にするとよい。  

#####仮想MACアドレス  
VRRPでは仮想MACアドレスも定義されている。  
フェイルオーバ時にはIPアドレスだけでなく、MACアドレスも一緒に引き継ぐ。これによりIPアドレス引き継ぎの際に通信相手がARPエントリを更新する必要性を排除している。  
...
負荷分散機能を提供するLinuxのモジュール  
「L4スイッチ相当の機能」が実装されている。  
IPVSの機能は以下のソフトウェアから利用することができる。  
* [ipvsadm](http://www.linuxvirtualserver.org/software/ipvs.html)  
* [keepalived](http://www.keepalived.org/)  

#####ipvsadm  
IPVSの開発元が提供しているコマンドラインツール  
仮想サーバの定義やリアルサーバの割り当てができる他、設定内容の確認や接続状況の確認もできる。転送レートなどの統計情報も表示できる。  

#####keepalived  
C言語で書かれているデーモン  
設定ファイル（/etc/keepalived/keepalived.conf）の内容に従ってIPVSの仮想サーバを構築する。  
さらにリアルサーバのヘルスチェックをしてダウンしたサーバを自動的に負荷分散から外したり、すべてのリアルサーバがダウンした場合に「ただいま混み合っています」などのメッセージを出したりできる。  

######GARP（gratuitous ARP）の遅延送出  
keepalivedのVRRPはRFC3768に従った実装にはなっていない。  
この問題を解決するためにkeepalivedには「garp_master_delay」という設定項目により、通信相手に確実にARPエントリを更新してもらうために数秒待ってから再度GARPを送出する。
